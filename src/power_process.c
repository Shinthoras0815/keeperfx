...
void process_disease(struct Thing *creatng)
{
        ...
        for (long n = 0; n < AROUND_MAP_LENGTH; n++)
        {
            ...
            while (i != 0)
            {
                ...
                { // Apply the spell kind stored in 'active_disease_spell'.
                    apply_spell_effect_to_thing(thing, cctrl->active_disease_spell, cctrl->exp_level, creatng->owner, creatng);
    ...
    {
        apply_damage_to_thing_and_display_health(creatng, game.conf.rules[creatng->owner].magic.disease_lose_percentage_health * cctrl->max_health / 100, cctrl->disease_caster_plyridx, INVALID_THING);
    }
}
